...
- **company_profiles.jsonl** — Company Profiles (MOPS t05st03)
  - 2,315 companies (1,078 listed 上市 + 889 OTC 上櫃 + 348 emerging 興櫃;
  ...
  - Query it from the command line with `query_companies.py` — e.g.
    `python3 query_companies.py --chairman 洪裕鈞` or
    `python3 query_companies.py --name-starts-with 威`. See
    **[EXAMPLES.md](EXAMPLES.md)** for the full set of natural-language queries
    (CLI shortcuts + Python recipes for ad-hoc questions).
...
It downloads the **live** TWSE ISIN lists for listed (`strMode=2`), OTC
(`strMode=4`) and emerging (`strMode=5`) stocks, compares the current company
codes against the file, then fetches MOPS profiles for any **new** companies
and adds them. (The listed and OTC lists are sectioned — only the 股票 section
is taken; the emerging list is a flat stock list.)
